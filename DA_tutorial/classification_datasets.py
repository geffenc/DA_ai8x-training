...
class ClassificationDataset(Dataset):
    def __init__(self,img_dir_path,transform,subset=None,get_path=False):
        ...
        img_classes = sorted(next(os.walk(img_dir_path))[1])
...
class DomainAdaptationPairDataset(Dataset):
    def __init__(self,source_img_dir_path,target_img_dir_path,transform,shot,pair_factor=1,adv_stage=False,source_dataset=None,target_dataset=None,subset=None):
        ...
        # or create them from the provided paths
            ...
            source_img_classes = sorted(next(os.walk(source_img_dir_path))[1])
            target_img_classes = sorted(next(os.walk(target_img_dir_path))[1])
...
Parameters:
  img_dir_path - Full path to directory with the images. No class subfolders needed.
  transform -    Specifies the image format (size, RGB, etc.) and augmentations to use
'''
...
''' get the office dataset '''
def office_get_datasets(data, conf, load_train=True, load_val=True, load_test=True):
    (data_dir, args) = data
...
def pass_get_datasets(data, conf):
    (data_dir, args) = data
    ...
    # transforms for training
    train_transform = transforms.Compose([
        transforms.RandomResizedCrop((conf.dimensions[1],conf.dimensions[2])),
    ...
    train_dataset = PassDataset(data_dir,train_transform)
    
    return train_dataset



def asl_get_datasets(data, conf, load_train=True, load_val=True, load_test=True):
    (data_dir, args) = data
...
Parameters:
  ...
  get_path -     returns the image file name in addition to the image and label
'''
class ClassificationBBDataset(Dataset):
    def __init__(self,img_dir_path,transform,subset=None,get_path=False):
        self.img_dir_path = img_dir_path
        self.transform = transform
        self.subset = subset
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # get the file name based on the idx
            file_name = os.path.basename(self.imgs[idx])
            ...
            # get the point location info
            point = row['region_shape_attributes']

            # convert to json
            point_js = json.loads(point.to_string(index=False))

            # extract the centroid as a tensor
            centroid = [point_js['cx'], point_js['cy']]
            centroid = torch.tensor(centroid).float()

            # return the sample (img (tensor)), object class (int), and the path optionally
            if self.get_path:
                return img, label, centroid, self.imgs[idx]
            else:
                return img, label, centroid
    ...
    def visualize_batch(self,model=None,device=None):
        ...
        # get the first batch
        (imgs, labels, centroids, paths) = next(iter(data_loader))
        imgs,labels,centroids = imgs.to(device), labels.to(device), centroids.to(device)
        preds = None
        ...
        imgs,labels,centroids = imgs.to("cpu"), labels.to("cpu"), centroid.to("cpu")
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                ax_array[i,j].imshow((imgs[idx].permute(1, 2, 0)+1)/2) # convert [-128/128, 127/128] to [0.0,1.0]
                ax_array[i,j].scatter(centroids[idx][0], centroids[idx][1], s=10, marker='.', c='r')
...
''' get the office dataset '''
def office_get_bbdatasets(data, conf, load_train=True, load_val=True, load_test=True):
    (data_dir, args) = data
    ...
    if load_train:
        ...
        train_dataset = ClassificationBBDataset(os.path.join(data_dir,"train"),train_transform,get_path=True)
        ...
        if load_val:
            ...
            train_dataset = ClassificationBBDataset(os.path.join(data_dir,"train"),train_transform,indices[val_size:],True)
            val_dataset = ClassificationBBDataset(os.path.join(data_dir,"train"),val_transform,indices[:val_size],True)
    ...
    if load_test:
        ...
        test_dataset = ClassificationBBDataset(os.path.join(data_dir,"test"),test_transform,None,True)
